...
Value darksend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "darksend <Advantageaddress> <amount>\n"
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "  stop         - Stop masternode configured in Advantage.conf\n"
                ...
                "  vote-many    - Vote on a Advantage initiative\n"
                "  vote         - Vote on a Advantage initiative\n"
                );
    ...
    if (strCommand == "genkey")
    {
        ...
        return CAdvantagecoinSecret(secret).ToString();
    }
...
Value masternodelist(const Array& params, bool fHelp)
{
                ...
                && strMode != "pubkey" && strMode != "rank" && strMode != "status" && strMode != "addr" && strMode != "votes" && strMode != "lastpaid"))
    {
        throw runtime_error(
                ...
                "  votes          - Print all masternode votes for a Advantage initiative (can be additionally filtered, partial match)\n"
