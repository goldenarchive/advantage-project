...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeAdvantageAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeAdvantageAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAmountAndRounds = strAnonymizeAdvantageAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeAdvantageAmount*COIN) nMaxToAnonymize = nAnonymizeAdvantageAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeAdvantageAmount * COIN) {
