...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeAdvantageAmount"))
        settings.setValue("nAnonymizeAdvantageAmount", 1000);
    nAnonymizeAdvantageAmount = settings.value("nAnonymizeAdvantageAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeAdvantageAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeAdvantageAmount").toString().toStdString());
...
//    if (!settings.contains("nTransactionFee")) {
        ...
        // cout << settings.value("nTransactionFee").toLongLong() << endl;
    // }
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeAdvantageAmount:
            return QVariant(nAnonymizeAdvantageAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeAdvantageAmount:
            nAnonymizeAdvantageAmount = value.toInt();
            settings.setValue("nAnonymizeAdvantageAmount", nAnonymizeAdvantageAmount);
            emit AnonymizeAdvantageAmountChanged(nAnonymizeAdvantageAmount);
            break;
        default:
            break;
        }
