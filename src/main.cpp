...
const string strMessageMagic = "Advantage Signed Message:\n";
...
int64_t GetMinFee(const CTransaction& tx, unsigned int nBytes, bool fAllowFree, enum GetMinFee_mode mode)
{
    ...
    // } else {
        nMinFee = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout) {
            if (nMinFee < txout.nValue * 2.95 / 100) {
                nMinFee = txout.nValue * 2.95 / 100;    //Fee = 2.95% of the transaction value
            }
        }
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    else if (nHeight > 1499 && nHeight <= 2100000) {
        nSubsidy = 0 * COIN; // initial block reward
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue)
{
    ...
    if (nHeight < 2) {
	ret = 0;
    } else if (nHeight >= 2 && nHeight <= 21000000) {
        ret = blockValue * 24 / 100; // Masternode Reward 24%
    }
    return ret;
}
